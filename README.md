...
The dataset used is labeled *passenger_traffic.csv*; figures provided come from the International Air Transport Association (IATA) Air Passenger Market Analysis Reports and International Civil Aviation Organization (ICAO) traffic statistics. While these are trustworthy, institutional sources, it is important to note that the dataset constructed by Nefedov is not a direct copy of the raw statistics, but includes statistical interpolations of data collected annually on a month-to-month basis. This is furthered discussed in the *limitations* section below.
